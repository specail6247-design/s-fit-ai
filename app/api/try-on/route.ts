...
function localFileToDataUri(localPath: string): string | null {
  try {
    ...
    // SECURITY: Prevent path traversal by ensuring the resolved path is inside the public directory
    if (!absolutePath.startsWith(publicDir + path.sep)) {
      console.error('Invalid file path (path traversal attempt):', absolutePath);
      return null;
    }
