...
function localFileToDataUri(localPath: string): string | null {
  try {
    ...
    // Prevent path traversal by ensuring the resolved path stays within the public directory
    if (!absolutePath.startsWith(publicDir + path.sep)) {
      console.error('Invalid path (potential path traversal):', absolutePath);
      return null;
    }
