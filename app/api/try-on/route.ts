...
function localFileToDataUri(localPath: string): string | null {
  try {
    ...
    // Security: Prevent path traversal by ensuring the resolved path is strictly within the public directory
    if (!absolutePath.startsWith(publicDir + path.sep)) {
      console.error('Security Error: Path traversal attempt detected.', { path: localPath });
      return null;
    }
