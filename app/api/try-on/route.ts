...
function localFileToDataUri(localPath: string): string | null {
  try {
    ...
    // SECURITY FIX: Prevent path traversal by strictly resolving and validating the target path
    const publicDir = path.join(process.cwd(), 'public');
    const absolutePath = path.resolve(publicDir, relativePath);

    // Ensure the resolved path actually resides inside the public directory
    if (!absolutePath.startsWith(publicDir + path.sep) && absolutePath !== publicDir) {
      console.error('Security violation: Attempted path traversal out of public directory:', absolutePath);
      return null;
    }
