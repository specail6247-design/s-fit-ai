...
function localFileToDataUri(localPath: string): string | null {
  try {
    ...
    // Prevent path traversal by ensuring the resolved path is within the public directory
    if (!absolutePath.startsWith(publicDir + path.sep)) {
      console.error('Security concern: Path traversal attempt detected.', { localPath });
      return null;
    }
