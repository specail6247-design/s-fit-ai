...
function localFileToDataUri(localPath: string): string | null {
  try {
    ...
    // Security: Prevent path traversal vulnerabilities by ensuring the resolved path
    // stays strictly within the intended public directory.
    if (!absolutePath.startsWith(publicDir + path.sep)) {
      console.error('Security error: Path traversal attempt blocked:', absolutePath);
      return null;
    }
