...
function localFileToDataUri(localPath: string): string | null {
  try {
    ...
    // SECURITY: Prevent path traversal by resolving absolute path and verifying it stays within public dir
    const publicDir = path.resolve(process.cwd(), 'public');
    const absolutePath = path.resolve(publicDir, relativePath);

    // Ensure the path strictly starts with public directory + path separator to prevent partial path bypass
    if (!absolutePath.startsWith(publicDir + path.sep) && absolutePath !== publicDir) {
      console.error('Security error: Invalid path traversal attempt:', absolutePath);
      return null;
    }
