...
function localFileToDataUri(localPath: string): string | null {
  try {
    ...
    // Security check: Prevent path traversal outside the public directory
    if (!absolutePath.startsWith(publicDir + path.sep)) {
      console.error('Security error: Invalid path, potential path traversal attempt:', localPath);
      return null;
    }
