...
export class LayeringEngine {
   ...
   * Uses the item's explicit zIndex if available, otherwise falls back to category defaults.
   */
  ...
  /**
   * Generates a masking strategy for the VTON process.
   * Determines which items need inpainting versus full-body generation.
   */
  generateCompositeStrategy(items: ClothingItem[]): LayeredComposition {
    ...
    return {
      layers: sorted.map(item => ({
        ...
        // Accessories usually need specific masking/inpainting rather than whole-body gen
  ...
  /**
   * Detects logical conflicts (e.g., Dress + Top + Bottom).
   * Returns warnings or resolves automatically.
   */
