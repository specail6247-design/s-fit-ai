...
export async function analyzeClothingStyle(imageUrl: string): Promise<ClothingStyleAnalysis> {
  ...
  const baseUrl = typeof window === 'undefined' ? (process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000') : window.location.origin;
  ...
  if (!response.ok) {
    ...
    throw new Error(errorData.error || `Vision API failed with status ${response.status}`);
  }
