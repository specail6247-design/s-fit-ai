...
/**
 * Deep Analysis using GPT-4o Vision (Server-side proxy)
 */
export async function analyzeClothingStyle(imageUrl: string): Promise<ClothingStyleAnalysis> {
  console.log("Requesting Server-side Deep Vision Analysis for image:", imageUrl.substring(0, 50) + "...");
  ...
  try {
    ...
    return data as ClothingStyleAnalysis;
  } catch (error) {
    console.error("Deep Analysis request failed:", error);
    // Fallback object to match original mock structure if server fails
    return {
      category: 'tops',
