...
export async function analyzeClothingStyle(imageUrl: string): Promise<ClothingStyleAnalysis> {
  ...
  try {
    ...
    console.warn("Failed to fetch from API, falling back to mock analysis:", error);
    // Fallback to mock analysis if API fails (e.g., during tests or if API route is unreachable)
