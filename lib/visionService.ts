...
export async function analyzeClothingStyle(imageUrl: string): Promise<ClothingStyleAnalysis> {
  ...
  // SECURITY (Sentinel): Removed client-side OpenAI instantiation to prevent API key exposure.
  // Analysis logic must be executed entirely server-side (e.g. via Next.js API Routes).
  // Returning mock analysis for demonstration purposes.
