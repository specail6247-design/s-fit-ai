    ...
    while (attempts < maxAttempts) {
      ...
      if (status === 'SUCCEEDED') {
        ...
        if (output && output.length > 0) {
          ...
          return {
            success: false,
            error: "Task succeeded but no video URL found in output"
          };
        }
        ...
        return {
          success: false,
          error: "Video generation task was cancelled"
        };
      }
