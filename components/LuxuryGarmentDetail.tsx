...
export default function LuxuryGarmentDetail() {
  ...
  useEffect(() => {
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (target.closest("button") || target.closest("a") || target.closest(".group")) {
        cursor.classList.add("scale-150", "border-[#ecab13]", "bg-[#ecab13]/10"); cursor.classList.remove("border-white/50", "bg-transparent");
      } else {
        cursor.classList.remove("scale-150", "border-[#ecab13]", "bg-[#ecab13]/10"); cursor.classList.add("border-white/50", "bg-transparent");
      }
    };
  ...
  return (
    <div className="min-h-screen bg-[#f8f7f6] dark:bg-[#0a0a0a] text-slate-900 dark:text-white font-sans relative">
      ...
      <main className="max-w-md mx-auto pt-16 pb-32">
        ...
        {/* Material Stats */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="px-4 -mt-4 relative z-10">
        ...
        {/* Comparison Table */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="px-4 py-4 bg-[#1a1a1a]/30 border-y border-[#2d2d2d] mb-8">
