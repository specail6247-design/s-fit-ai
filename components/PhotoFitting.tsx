...
export default function PhotoFitting() {
  ...
  // Toggle immersive mode (Digital Mirror)
  const toggleImmersive = () => {
    setIsFitting(!isFitting);
  };

  const uiClass = `transition-opacity duration-700 ease-in-out pointer-events-none ${
    isFitting ? "opacity-0" : "opacity-100"
  }`;
  ...
  return (
    <div className={`relative flex h-screen w-full flex-col overflow-hidden bg-[#f5f6f8] text-white dark:bg-[#101622] ${spaceGrotesk.className}`}>
      {/* Background Click Area for toggling Immersive Mode */}
      <div
        className="absolute inset-0 z-10 cursor-pointer"
        onClick={toggleImmersive}
        title="Tap to toggle Digital Mirror"
      ></div>

      {/* Top App Bar */}
      <div className={`z-50 flex items-center justify-between bg-transparent p-4 ${uiClass}`}>
        <div className="pointer-events-auto flex size-12 shrink-0 items-center justify-center rounded-full bg-[#101622]/40 text-white backdrop-blur-md hover:bg-[#101622]/60 transition-colors">
        ...
        <div className="flex w-12 items-center justify-end">
          <button className="pointer-events-auto flex size-12 cursor-pointer items-center justify-center rounded-full bg-[#101622]/40 text-white backdrop-blur-md hover:bg-[#101622]/60 transition-colors">
      ...
      <div className="absolute inset-0 z-0">
        ...
        >
          {/* Scanning Effect - Only show when processing */}
          {isProcessing && (
            <div
              className={`absolute top-[40%] z-20 h-[2px] w-full ${uiClass}`}
              style={{
          ...
          {/* HUD Overlays */}
          <div className={`glass-panel absolute left-4 top-24 rounded-lg p-3 ${uiClass}`} style={{ background: "rgba(16, 22, 35, 0.8)", backdropFilter: "blur(12px)", border: "1px solid rgba(49, 67, 104, 0.5)" }}>
          ...
          <div className={`glass-panel absolute right-4 top-24 rounded-lg p-3 text-right ${uiClass}`} style={{ background: "rgba(16, 22, 35, 0.8)", backdropFilter: "blur(12px)", border: "1px solid rgba(49, 67, 104, 0.5)" }}>
      ...
      {isProcessing && (
        <div className={`absolute inset-x-0 top-1/2 z-30 -translate-y-1/2 px-6 ${uiClass}`}>
      ...
      {/* Controls Footer */}
      <div className={`mt-auto space-y-4 p-4 z-40 ${uiClass}`}>
        ...
        <div className="grid grid-cols-1 gap-3">
          <div className="glass-panel group flex items-center justify-between rounded-xl p-4 pointer-events-auto" style={{ background: "rgba(16, 22, 35, 0.8)", backdropFilter: "blur(12px)", border: "1px solid rgba(49, 67, 104, 0.5)" }}>
          ...
          <div className="glass-panel flex items-center justify-between rounded-xl p-4 pointer-events-auto" style={{ background: "rgba(16, 22, 35, 0.8)", backdropFilter: "blur(12px)", border: "1px solid rgba(49, 67, 104, 0.5)" }}>
            <div className="flex items-center gap-4">
              <div className="flex size-10 items-center justify-center rounded-lg bg-[#256af4]/20 text-[#256af4]">
        ...
        <button
          className="flex h-14 w-full items-center justify-center gap-2 rounded-xl bg-[#256af4] text-base font-bold text-white shadow-lg shadow-[#256af4]/20 transition-colors hover:bg-blue-600 pointer-events-auto"
          onClick={() => {
      ...
      {/* Heatmap Legend */}
      <div className={`glass-panel absolute bottom-64 left-4 z-40 flex flex-col gap-1.5 rounded-lg p-2 ${uiClass}`} style={{ background: "rgba(16, 22, 35, 0.8)", backdropFilter: "blur(12px)", border: "1px solid rgba(49, 67, 104, 0.5)" }}>
        <div className="flex items-center gap-2">
