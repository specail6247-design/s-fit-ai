  ...
  const handleShareToStory = async () => {
    ...
    try {
      ...
      canvas.toBlob(async (blob) => {
        ...
        if (!blob) return;
        const file = new File([blob], 'sfit-story.png', {
 type: 'image/png' });
...
(e) => handleFileUpload(e, setUserImage)} className="hidden" id="user-upload" />
...
/* Garment Input */}
          <div className="space-y-2">
            ...
            <div className="border border-white/20 bg-black/40 rounded-xl p-4 hover:border-[#007AFF] transition-colors group">
              <input type="file" onChange={
(e) => handleFileUpload(e, setGarmentImage)} className="hidden" id="garment-upload" />
...
/* Footer Links */}
        <div className="mt-auto pt-8 flex justify-between text-[10px] text-gray-500 font-mono relative z-10">
          <button onClick={
() => setIsPrivacyOpen(true)} className="hover:text-white transition-colors">Privacy & Terms</button>
          <button onClick={
() => setIsSupportOpen(true)} className="hover:text-white transition-colors">Support Hub</button>
        </div>
      </div>
...
() => window.location.reload()}
        ...
        <PrivacyModal isOpen={
isPrivacyOpen} onClose={
() => setIsPrivacyOpen(false)} />
        <SupportHub isOpen={
isSupportOpen} onClose={
