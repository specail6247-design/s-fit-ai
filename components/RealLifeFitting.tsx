    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans flex overflow-hidden">
      ...
      <div className="w-1/3 min-w-[400px] h-full p-8 flex flex-col z-10 glass-panel border-r border-white/10 relative">
        ...
        <header className="mb-10 relative z-10">
          <div className="absolute top-1 right-0 z-20 flex gap-4 items-center">
            <button
              onClick={() => setLoginModalOpen(true)}
              className="text-[10px] font-bold text-white/50 hover:text-white transition-colors tracking-widest uppercase"
            >
              Access
            </button>
            <button
              onClick={() => setSupportHubOpen(true)}
              className="w-6 h-6 rounded-full border border-white/20 flex items-center justify-center text-[10px] text-white/50 hover:text-white hover:border-white transition-all"
            >
              ?
            </button>
          </div>
