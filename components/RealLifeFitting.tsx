...
export default function RealLifeFitting() {
  ...
  // Initialize and manage audio based on processing state
  useEffect(() => {
    if (isProcessing && !isMuted) {
      ...
      oscillator.frequency.setValueAtTime(65, ctx.currentTime); // Low hum frequency (e.g., C2)

      gainNode.gain.setValueAtTime(0, ctx.currentTime);
      gainNode.gain.linearRampToValueAtTime(0.1, ctx.currentTime + 1); // Fade in
    ...
    } else if ((!isProcessing || isMuted) && gainNodeRef.current && audioCtxRef.current) {
      const ctx = audioCtxRef.current;
      gainNodeRef.current.gain.linearRampToValueAtTime(0, ctx.currentTime + 0.5); // Fade out
      ...
      <div className="w-1/3 min-w-[400px] h-full p-8 flex flex-col z-10 glass-panel border-r border-white/10 relative">
        ...
        <header className="mb-10 relative z-10 flex justify-between items-start">
          ...
          <button
            ...
            className="flex items-center justify-center p-2 rounded-full border border-white/20 hover:bg-white/10 text-gray-400 hover:text-white transition-colors"
              ...
              <label htmlFor="garment-upload" className="cursor-pointer flex items-center gap-4">
                <div className="w-16 h-16 bg-gray-800 rounded-lg flex items-center justify-center overflow-hidden border border-white/10">
                  {garmentImage ? (
                    <>
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={garmentImage} alt="Garment photo" className="w-full h-full object-cover" />
                    </>
                  ) : <span className="text-2xl">👕</span>}
                </div>
                <div>
