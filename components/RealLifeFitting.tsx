...
import { ShareToStoryModal } from './ui/ShareToStoryModal';
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans flex overflow-hidden">
      ...
      <div className="w-1/3 min-w-[400px] h-full p-8 flex flex-col z-10 glass-panel border-r border-white/10 relative">
        ...
        <div className="space-y-8 relative z-10 flex-1 overflow-y-auto pb-4">
          {/* User Photo Input */}
          <div className="space-y-2">
            <div className="flex justify-between items-end">
              <label className="text-xs font-bold text-[#007AFF] uppercase">01. Identification</label>
              <div className="flex items-center gap-1 text-[9px] text-green-400 bg-green-400/10 px-2 py-0.5 rounded-full border border-green-400/20">
                <span>🛡️</span> Photos securely processed
              </div>
            </div>
            ...
            <p className="text-[9px] text-gray-500 text-right">Data is not shared and deleted after session.</p>
          </div>
        ...
        <div className="mt-8 relative z-10">
          ...
          <div className="mt-8 pt-4 border-t border-white/10 flex justify-between items-center text-[10px] text-gray-500">
            ...
            <button onClick={() => setShowReport(true)} className="hover:text-white transition-colors flex items-center gap-1">
          ...
          >
            <div className="relative group">
              ...
              <button
                ...
                className="absolute bottom-4 right-4 bg-gradient-to-r from-[#833AB4] to-[#F77737] text-white px-3 py-1.5 rounded-lg text-xs font-bold shadow-lg hover:opacity-90 transition-all flex items-center gap-1.5"
              >
                Share to Story <span>📸</span>
              </button>
            </div>
          </motion.div>
        )}
      ...
      <ShareToStoryModal isOpen={showStory} onClose={() => setShowStory(false)} imageUrl={resultImage} />
    </div>
  );
}
