    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto w-full px-4">
      {modes.map((mode, index) => (
        <motion.button
          key={mode.id}
          type="button"
          aria-label={`Select ${mode.title} mode: ${mode.subtitle}`}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: index * 0.1 }}
          className="mode-card group relative flex flex-col items-center text-center h-[400px] justify-between focus:outline-none focus-visible:ring-2 focus-visible:ring-cyber-lime focus-visible:ring-offset-2 focus-visible:ring-offset-void-black"
