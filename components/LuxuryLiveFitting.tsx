...
export default function LuxuryLiveFitting() {
  ...
  return (
    <div className={`relative flex h-screen w-full flex-col overflow-hidden bg-[#f6f7f8] text-white dark:bg-[#0a0a0a] ${cinzel.className}`}>
      {/* Custom Gold Ring Cursor */}
      <motion.div
        className="pointer-events-none fixed left-0 top-0 z-[100] h-8 w-8 rounded-full border border-[#ecab13] mix-blend-difference"
        animate={{
      ...
      />
      <motion.div
        className="pointer-events-none fixed left-0 top-0 z-[100] h-2 w-2 rounded-full bg-[#ecab13] mix-blend-difference"
        animate={{
          x: mousePosition.x - 4,
          y: mousePosition.y - 4,
        }}
          ...
          >
            <div className="relative h-24 w-24">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-[10px] uppercase tracking-[0.3em] text-[#ecab13]">S_FIT</span>
              </div>
            </div>
            <motion.p
              ...
              className="mt-8 text-[10px] uppercase tracking-[0.2em] text-white/50"
            >
      ...
      {/* Brand Parallax Background (Overrides main bg if brand selected) */}
      {brandData?.bannerImage && (
        <motion.div
          className="absolute inset-0 z-0 bg-cover bg-center opacity-30"
          style={{ backgroundImage: `url('${brandData.bannerImage}')` }}
          initial={{ scale: 1.1 }}
          animate={{ scale: 1 }}
      ...
      {brandData?.description && (
        <div className="absolute top-24 left-1/2 -translate-x-1/2 z-10 w-full max-w-md px-4 text-center">
          <motion.div
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 2.5, duration: 1 }}
            className="rounded-xl border border-[#ecab13]/30 bg-black/40 p-4 backdrop-blur-md"
          >
            <h3 className="mb-2 text-sm font-bold uppercase tracking-[0.2em] text-[#ecab13]">{brandData.name} Experience</h3>
            <p className="text-xs leading-relaxed text-white/80">{brandData.description}</p>
          </motion.div>
        </div>
      )}
      ...
      >
        ...
        <div className="mt-auto pb-10 z-10">
          {/* Garment Carousel */}
          <div className="flex overflow-x-auto px-4 py-8 space-x-6 scrollbar-hide" style={{ maskImage: "linear-gradient(to right, transparent, black 10%, black 90%, transparent)" }}>
            <div className="flex items-stretch gap-6">
              {/* Selected Garment */}
              <div className="flex min-w-[160px] flex-col gap-4 rounded-xl border border-[#ecab13] bg-[#ecab13]/10 p-2 backdrop-blur-md transition-all duration-700">
                <div className="relative aspect-[3/4] w-full rounded-lg overflow-hidden bg-black/40">
                  <LuxuryImageDistortion
                    ...
                    alt="Luxury gold blazer thumbnail"
                    className="w-full h-full object-cover"
                  />
                </div>
                <div className="px-2 pb-2 text-center">
                  <p className="truncate text-sm font-bold uppercase tracking-widest text-white">Aura Blazer</p>
                  <p className="text-xs font-bold text-[#ecab13] mt-1">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(2400)}</p>
                </div>
              </div>
              ...
              {[
                ...
                <div key={i} className="flex min-w-[160px] flex-col gap-4 rounded-xl p-2 opacity-60 hover:opacity-100 transition-all duration-1000" style={{ background: "rgba(10, 10, 10, 0.6)", backdropFilter: "blur(12px)", border: "1px solid rgba(255, 255, 255, 0.1)" }}>
                    <div className="relative aspect-[3/4] w-full rounded-lg overflow-hidden bg-black/40">
                      <LuxuryImageDistortion
                        imageUrl={item.img}
                        alt={item.name}
                    ...
                    <div className="px-2 pb-2 text-center">
                    <p className="truncate text-sm font-bold uppercase tracking-widest text-white">{item.name}</p>
                    <p className="text-xs font-bold text-white/50 mt-1">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(item.price)}</p>
                    </div>
                </div>
              ))}
            </div>
