...
export default function LuxuryLiveFitting() {
  ...
  const [activeBrand] = useState("GUCCI");
  ...
  const loaderVariants: Variants = {
    ...
    return (
    <div className={`relative flex h-screen w-full flex-col overflow-hidden bg-[#f6f7f8] text-white dark:bg-[#101922] ${geist.className}`}>
      {/* Main AR Viewport Container */}
      {/* Custom Cursor */}
      <motion.div
        className="pointer-events-none fixed z-[9999] size-8 rounded-full border border-[#ecab13] mix-blend-difference"
      ...
      {isLoading && (
        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-[#101922]">
          <div className="relative size-32">
            ...
            <div className={`absolute inset-0 flex items-center justify-center text-xs font-light tracking-[0.3em] text-[#ecab13] ${playfair.className}`}>
              S_FIT
            </div>
          </div>
        </div>
      ...
      >
        ...
        <div className="absolute top-24 left-0 w-full overflow-hidden h-32 opacity-20 pointer-events-none flex items-center justify-center">
            <motion.h1
                initial={{ y: 50, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ duration: 1.5, ease: "easeOut" }}
                className={`text-[8rem] whitespace-nowrap text-white ${playfair.className}`}
            >
                {activeBrand}
            </motion.h1>
        </div>
        ...
        <div className="z-10 flex items-center justify-between p-4 pt-8">
          <div className="flex size-12 items-center justify-center rounded-full" style={{ background: "rgba(16, 25, 34, 0.6)", backdropFilter: "blur(12px)", border: "1px solid rgba(236, 171, 19, 0.2)" }}>
            <span className="material-symbols-outlined text-white">close</span>
          </div>
          <div className="flex items-center gap-2 rounded-full px-4 py-2" style={{ background: "rgba(16, 25, 34, 0.6)", backdropFilter: "blur(12px)", border: "1px solid rgba(236, 171, 19, 0.2)" }}>
            <div className="size-2 animate-pulse rounded-full bg-[#ecab13]"></div>
            <h2 className="text-sm font-bold tracking-widest uppercase text-[#ecab13] ${playfair.className}">Live Fit AI</h2>
          </div>
          <div className="flex size-12 items-center justify-center rounded-full" style={{ background: "rgba(16, 25, 34, 0.6)", backdropFilter: "blur(12px)", border: "1px solid rgba(236, 171, 19, 0.2)" }}>
        ...
        <div className="mt-4 space-y-4 px-4 z-10">
          <div className="max-w-[240px] rounded-xl p-4" style={{ background: "rgba(16, 25, 34, 0.6)", backdropFilter: "blur(12px)", border: "1px solid rgba(236, 171, 19, 0.2)" }}>
        ...
        <div className="absolute right-4 top-1/4 z-10 flex flex-col gap-3">
          <div className="flex min-w-[120px] flex-col gap-1 rounded-lg p-3" style={{ background: "rgba(16, 25, 34, 0.6)", backdropFilter: "blur(12px)", border: "1px solid rgba(236, 171, 19, 0.2)" }}>
        ...
        <div className="mt-auto pb-10 z-10">
          ...
          <div className="flex overflow-x-auto px-2 py-4 scrollbar-hide" style={{ maskImage: "linear-gradient(to right, transparent, black 10%, black 90%, transparent)" }}>
            <div className="flex items-stretch gap-3">
              <div className="flex min-w-28 flex-col gap-2 rounded-xl border-2 border-[#ecab13] bg-[#ecab13]/20 p-1 backdrop-blur-md">
                <div
              ...
              {[
                ...
                <div key={i} className="flex min-w-28 flex-col gap-2 rounded-xl p-1 opacity-80" style={{ background: "rgba(16, 25, 34, 0.6)", backdropFilter: "blur(12px)", border: "1px solid rgba(236, 171, 19, 0.2)" }}>
                    <div
                    className="aspect-[4/5] w-full rounded-lg bg-cover bg-center bg-no-repeat"
          ...
          <div className="flex items-center justify-center gap-10 p-4">
            <button className="flex size-12 shrink-0 items-center justify-center rounded-full text-white" style={{ background: "rgba(16, 25, 34, 0.6)", backdropFilter: "blur(12px)", border: "1px solid rgba(236, 171, 19, 0.2)" }}>
