  ...
  return (
    <div className="w-full h-full flex flex-col bg-void-black text-pure-white">
      <div className="flex-1 relative min-h-[350px]">
        ...
        {/* Controls and Overlays - Fades out during AI Analysis for 'Digital Mirror' effect */}
        <AnimatePresence>
            {!aiTryOnLoading && (
                <motion.div
                ...
                >
                    ...
                    {/* Rotation hint */}
                    {!webglFailed && (
                    <motion.div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 text-soft-gray/60 text-xs bg-void-black/50 px-3 py-1 rounded-full"
                    ...
                    {topPicks.length > 0 && (
                    <div className="absolute top-16 left-1/2 -translate-x-1/2 w-[90%] max-w-md pointer-events-auto">
                        {isMiniBarCollapsed ? (
      ...
      {/* Item Selector Footer - Fades out during AI Analysis */}
      <AnimatePresence>
        {!aiTryOnLoading && (
            <motion.div
