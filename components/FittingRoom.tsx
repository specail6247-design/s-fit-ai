...
export function FittingRoom() {
  ...
  const isImmersiveHidden = aiTryOnLoading;
  const immersiveClass = `transition-opacity duration-700 ease-in-out ${isImmersiveHidden ? 'opacity-0 pointer-events-none' : 'opacity-100'}`;
  ...
  return (
    <div className="w-full h-full flex flex-col bg-void-black text-pure-white">
      <div className="flex-1 relative min-h-[350px]">
        ...
        {/* Controls Overlay */}
        <div className={`absolute top-4 right-4 flex flex-col gap-2 z-10 ${immersiveClass}`}>
        ...
        <div className={`absolute top-4 left-4 flex gap-2 z-20 ${immersiveClass}`}>
        ...
        {topPicks.length > 0 && (
          <div className={`absolute top-16 left-1/2 -translate-x-1/2 w-[90%] max-w-md z-20 ${immersiveClass}`}>
            {isMiniBarCollapsed ? (
        ...
        <div className={`absolute bottom-4 left-4 right-4 z-10 pointer-events-none ${immersiveClass}`}>
            <AnimatePresence>
                {currentItem && poseAnalysis?.proportions && recommendedFit && !isImmersiveHidden && (
                    <motion.div 
      ...
      <div className={`p-4 border-t border-border-color bg-void-black ${immersiveClass}`}>
      ...
      {currentItem && (
        <div className={`p-4 bg-charcoal/30 border-t border-border-color ${immersiveClass}`}>
          <div className="flex items-center justify-between mb-3">
