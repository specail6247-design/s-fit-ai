...
function Mannequin({ 
  ...
  useFrame((state) => {
    if (meshRef.current) meshRef.current.rotation.y = Math.sin(state.clock.elapsedTime * 0.3) * 0.05;
  });

  const skinMaterial = (
    <meshStandardMaterial color="#e0b8a0" roughness={0.6} transparent={opacity < 1.0} opacity={opacity} />
  );
  ...
  return (
    <>
      ...
      <group position={mannequinPosition} scale={[scale, scale, scale]}>
        <Mannequin height={height} bodyShape={userStats?.bodyShape} proportions={poseAnalysis?.proportions} opacity={selectedMode === 'digital-twin' ? 0.2 : 1.0} />
