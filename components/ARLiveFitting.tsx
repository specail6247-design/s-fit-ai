            ...
            <div className="flex items-stretch gap-3">
              {luxuryItems.map((item) => (
                <div
                    ...
                    className={`flex min-w-28 flex-col gap-2 rounded-xl p-1 backdrop-blur-md transition-all cursor-pointer relative overflow-hidden group
                        ${selectedItem?.id === item.id
                            ? "border-2 border-[#2b8cee] bg-[#2b8cee]/20 opacity-100 scale-105"
                            : "border border-white/10 bg-[rgba(16,25,34,0.6)] opacity-80 hover:opacity-100"}`}
                >
                    <div className="relative aspect-[4/5] w-full rounded-lg overflow-hidden bg-black/20">
                        <Image
                            src={item.imageUrl}
                            alt={item.name}
                            fill
                            ...
                            onError={(e) => {
                                ...
                                target.src = "https://placehold.co/400x500/1a1a1a/white?text=No+Image";
                            }}
                        />
                        {/* Shimmer Effect for Selected Item */}
                        {selectedItem?.id === item.id && (
                            <div className="absolute inset-0 luxury-shimmer pointer-events-none"></div>
                        )}
                    </div>
                    <div className="px-1 pb-1">
                        <p className="truncate text-[10px] font-bold uppercase text-white">{item.name}</p>
                        <p className={`text-[10px] font-bold ${selectedItem?.id === item.id ? "text-[#2b8cee]" : "text-white/50"}`}>
